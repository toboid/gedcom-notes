...
args_parser = argparse.ArgumentParser(description='Creates obsidian notes from a gedcom file.')
args_parser.add_argument('--file_path', type=str, required=True, help='The path to the gedcom input file')
args_parser.add_argument('--output_dir', type=str, required=True, help='The directory to output the notes')
...
for individual in tree.individuals:
    ...
    with ObsidianNote(individual.get_display_name(), output_dir) as note:
        ...
        for parent in individual.get_parents():
            note.write_meta_hidden(
                'Parent', note.linked_note(parent.get_display_name()))
        note.write_line('')

        note.write_h2('Children')
        for child in individual.get_children():
            note.write_meta_hidden(
                'Child', note.linked_note(child.get_display_name()))
        note.write_line('')

        note.write_line('Partners')
        for partner in individual.get_partners():
            note.write_line(partner.get_display_name())
